...
import cv2
import math
from RaDL.radl.radl_arch_up import RaDL, NaiveFuser
...
class RaDLProcessor(nn.Module):
    def __init__(self, config, attnstore, place_in_unet):
        super().__init__()
        self.attnstore = attnstore
        self.place_in_unet = place_in_unet
        self.not_use_radl = config['not_use_radl']
        self.naive_fuser = NaiveFuser()        
        self.embedding = {}
        
        # Initialize other_info with default values
        self.other_info = {
            'verbs': None,
            'text_encoder': None,
            'tokenizer': None
        ...
        if not self.not_use_radl:
            self.radl = RaDL(config['C'])
    ...
    def __call__(
        ...
        if ith > RaDLsteps:
            ...
            not_use_radl = self.not_use_radl
        is_vanilla_cross = (not_use_radl and ith > NaiveFuserSteps)
        if instance_num == 0:
        ...
        # In this case, we need to use radl or naive_fuser, so we copy the hidden_states_cond (instance_num+1) times for QKV
        ...
        other_info['verbs'] = verbs
        other_info['text_encoder'] = attn.to_q.weight.device
        other_info['tokenizer'] = attn.to_q.weight.device

        if not not_use_radl:
            hidden_states_cond, fuser_info = self.radl(cond_ca_output,
                                        guidance_masks,
...
class StableDiffusionRaDLPipeline(StableDiffusionPipeline):
    def __init__(
            self,
            vae: AutoencoderKL,
    ...
    def __call__(
        ...
        Args:
            ...
            verbs (`List[str]`, *optional*):
                The list of verbs to be used in the global prompt.
        ...
        """
        def aug_phase_with_and_function(phase, instance_num, verb=None):
            instance_num = min(instance_num, 7)
            copy_phase = [phase] * instance_num
            
            if verb is not None:
                copy_phase = [f"{p} {verb}" for p in copy_phase]
        ...
        if aug_phase_with_and:
            ...
            for i in range(1, len(prompt[0])):
                ...
                current_verb = verbs[i-1] if verbs is not None and i-1 < len(verbs) else None
                
                prompt[0][i] = aug_phase_with_and_function(
                    prompt[0][i],
                    instance_num,
                    verb=current_verb
                )
        ...
        def create_global_prompt(instance_prompts, verbs=None):
            
            components = []
            for i, inst_prompt in enumerate(instance_prompts):
                if verbs and i < len(verbs):
                    components.append(f"{inst_prompt} {verbs[i]}")
                else:
                    components.append(inst_prompt)
            
            global_prompt = "a high quality image of " + ", and ".join(components)
            return global_prompt

        if prompt is not None and isinstance(prompt, list):
            for batch_idx in range(len(prompt)):
                instance_prompts = prompt[batch_idx][1:]  # 첫 번째 요소(기존 글로벌 프롬프트)를 제외한 인스턴스 프롬프트들
                new_global_prompt = create_global_prompt(
                    instance_prompts,
                    verbs=verbs
                )
                prompt[batch_idx][0] = new_global_prompt  # 기존 글로벌 프롬프트를 새로운 것으로 교체
        ...
        # Update cross_attention_kwargs to include verbs
