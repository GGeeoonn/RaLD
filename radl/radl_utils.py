...
from radl.radl_pipeline import StableDiffusionRaDLPipeline, RaDLProcessor, AttentionStore
...
def load_radl(unet, attention_store, pretrained_RaDL_path: Union[str, Dict[str, torch.Tensor]], attn_processor,
                        **kwargs):

    state_dict = torch.load(pretrained_RaDL_path, map_location="cpu")
    ...
    # change the key of RaDL.ckpt as the form of diffusers unet 
    for key, value in state_dict.items():
        key_list = key.split(".")
        assert 'radl' in key_list
        ...
        sub_key = '.'.join(key_list[key_list.index('radl'):])
        adapter_grouped_dict[attn_processor_key][sub_key] = value

    # Create RaDL Processor
    config = {'not_use_radl': False}
    for key, value_dict in adapter_grouped_dict.items():
        dim = value_dict['radl.norm.bias'].shape[0]
        config['C'] = dim
...
def offlinePipelineSetupWithSafeTensor(sd_safetensors_path):
    ...
    radl_ckpt_path = os.path.join(project_dir, 'pretrained_weights/radl_pretrained.ckpt')
    clip_model_path = os.path.join(project_dir, 'radl_gui_weights/clip/text_encoder')
    clip_tokenizer_path = os.path.join(project_dir, 'radl_gui_weights/clip/tokenizer')
    original_config_file = os.path.join(project_dir, 'radl_gui_weights/v1-inference.yaml')
    ...
    pipe = StableDiffusionRaDLPipeline.from_single_file(sd_safetensors_path,
    ...
    from radl.radl_utils import load_radl
    load_radl(pipe.unet , pipe.attention_store,
            radl_ckpt_path, attn_processor=RaDLProcessor)
