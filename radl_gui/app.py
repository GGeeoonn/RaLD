...
from radl.radl_utils import offlinePipelineSetupWithSafeTensor, seed_everything
import json, time
...
def process_request(req_data):
    ...
    if sd_safetensors_name == "":
        GUI_progress[0] = 100
        return "ckpt_warning"
    sd_safetensors_path = os.path.join(project_dir, 'radl_gui_weights/sd',
                                    sd_safetensors_name)
    ...
    image = pipe(prompt_final, bboxes, 
                ...
                width=width, height=height, RaDLsteps=RaDLsteps, NaiveFuserSteps=NaiveFuserSteps,
...
def get_sd_ckpts():
    directory = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'radl_gui_weights/sd')
