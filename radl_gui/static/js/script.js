...
function buildPrompt() {
  ...
  if (State.checkpoint_name) {
    prompt["sd_checkpoint_name"] = State.checkpoint_name
  }
  else {
    prompt["sd_checkpoint_name"] = ""
    addToast("You have not selected a model yet")
  }
  ...
  State.boxMap.forEach((box, box_id, map) => {
    ...
    prompt["Instance"+String(instance_num)] = nodeRaDLTextboxApply(
        box.prompt,
        box.width,
        box.height,
        box.x,
        box.y,
        document.getElementById("ca-input-" + box_id).value,
        document.getElementById("ea-input-" + box_id).value,
        document.getElementById("sac-input-" + box_id).value
      );
    });
  ...
  prompt["positive_prompt"] = State.positive_conditioning.replace(
    /[\s;]+$/g,
    ""
  )
  tags = tags.join(";");
  ...
  console.log(State.negative_conditioning)

  prompt["negative_prompt"] = State.negative_conditioning.replace(
    /[\s;]+$/g,
    ""
  )
  ...
  prompt["num_inference_steps"] = State.steps
  prompt["RaDLsteps"] = State.RaDLsteps
  prompt["NaiveFuserSteps"] = State.NaiveFuserSteps
  prompt["seed"] = seed
  prompt["cfg"] = State.cfg
...
function generateImage() {
    console.log('try to get image from backend')
    let prompt = buildPrompt();
    fetch('/get_image', {
      method: 'POST',
      headers: {
    ...
    // Establish a connection to the server-sent event to listen for progress
    var evtSource = new EventSource("/GUI_progress");
    evtSource.onmessage = function(event) {
      var GUI_progress = JSON.parse(event.data).GUI_progress;
      document.getElementById('progress-bar').style.width = `${GUI_progress}%`;

      // When the progress reaches 100%, close the connection
      if(GUI_progress === 100) {
          evtSource.close();
      }
    };
...
window.addEventListener("load", () => {
  ...
  let RaDLsteps_input = document.getElementById("RaDLsteps");
  State.RaDLsteps = State.RaDLsteps || 15;
  RaDLsteps_input.value = State.RaDLsteps;
  RaDLsteps_input.addEventListener("input", (event) => {
    State.RaDLsteps = event.target.value;
  });
  ...
  document.getElementById("generate-layout").addEventListener("click", (event) => {
    let layout_info = State.positive_conditioning.replace(
      /[\s;]+$/g,
      ""
    )
    layout_info += ':\n'
    ...
    State.boxMap.forEach((box, box_id, map) => {
      // Initialize the array in LayoutMap for the box.prompt if it doesn't already exist
      if (!LayoutMap.has(box.prompt)) {
        LayoutMap.set(box.prompt, []);
      }
    
      // Calculate the normalized positions and dimensions
      let normalizedValues = [
        box.x / width.value, 
        box.y / height.value, 
        (box.x + box.width) / width.value, 
        (box.y + box.height) / height.value
      ];
    
      // Add the calculated values to the appropriate array in LayoutMap
      LayoutMap.get(box.prompt).push(normalizedValues);
    });
    for (let [key, value] of LayoutMap) {
      box_prompt = key
      layout_info = layout_info + ` ${box_prompt}:\n`
      console.log(66666666)
      console.log(value)
      for (let i = 0; i < value.length; i++) {
        layout_info = layout_info + `  - - ${value[i][0]}\n`
        layout_info = layout_info + `    - ${value[i][1]}\n`
        layout_info = layout_info + `    - ${value[i][2]}\n`
        layout_info = layout_info + `    - ${value[i][3]}\n`
      }
    }

    let layout_box = document.getElementById("output-layout")
    layout_box.value = layout_info;
  });
