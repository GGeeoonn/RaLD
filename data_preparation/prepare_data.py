...
def segment_from_file(jpg_dir = None, caption_path = None,rank_idx = None):
    ...
    for jpg_name in tqdm(image_list):
        ...
        origin_doc = nlp(caption)
        verbs = []
        verb_phrases = []

        for sent in origin_doc.sentences:
            words = sent.words
            i = 0
            while i < len(words):
                word = words[i]
                if word.upos == "VERB":
                    verb_phrase = word.text
                    # 동사 다음에 전치사/부사 등이 오면 묶어서 동사구로 간주
                    j = i + 1
                    while j < len(words) and words[j].upos in ["ADP", "PART", "ADV"]:
                        verb_phrase += f" {words[j].text}"
                        j += 1
        ...
        segment_label['verbs'] = verbs
        segment_label['verb_phrases'] = verb_phrases

        if jpg_name in result:
            result[jpg_name].append(segment_label)
        else:
            result[jpg_name] = [segment_label]
...
if __name__ == '__main__':
    ...
    '''
    segment_from_file(
        jpg_dir = "/root/RaDL/data_preparation/train2014",        
        caption_path = "/root/RaDL/data_preparation/annotations/captions_train2014.json"  # 캡션 JSON 파일 경로
    )